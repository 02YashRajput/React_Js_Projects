...
const Container = ({ itemName, data }) => {
    ...
    return (
        <div className='w-full h-[27rem] text-slate-500 px-5 flex flex-col justify-evenly relative'>
            ...
            <button ref={leftButton} onClick={handleScrollLeft} className="absolute top-1/2 -translate-y-1/2 translate-x-2 left-0   z-10 h-10 w-10 rounded-full flex items-center justify-center bg-[rgba(0,0,0,0.7)]  dark:bg-black dark:text-white"><FaChevronLeft /></button>
            <div ref={scrollRef} className='transition-all duration-800 ease-in-out overflow-x-scroll h-[22rem] overflow-y-hidden relative flex gap-5 hidingscrollbar'>
            ...
            <button ref={rightButton} onClick={handleScrollRight} className="absolute top-1/2 -translate-y-1/2 -translate-x-2 right-0  z-10 h-10 w-10 rounded-full flex items-center justify-center bg-[rgba(0,0,0,0.7)]  dark:bg-black dark:text-white"><FaChevronRight /></button>
        </div>
    );
}
